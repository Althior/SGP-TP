...
void ExceptionHandler(ExceptionType exceptiontype, int vaddr)
{
  ...
  case SYSCALL_EXCEPTION: {
    ...
    case SC_P:{
      ...
      Semaphore * sem = (Semaphore *) g_object_ids->SearchObject(sid);
      if (sem && sem->typeId == SEMAPHORE_TYPE_ID)
	{
	  sem->P();
	  g_syscall_error->SetMsg((char*)"",NoError);
	  g_machine->WriteIntRegister(2,0);
	...
	{
	  g_syscall_error->SetMsg(msg, InvalidSemaphoreId);
	  g_machine->WriteIntRegister(2,-1);
	}
      break;
    }
    ...
    case SC_SEM_CREATE:{
      DEBUG('e', (char*)"SEMAPHORE: SemCreate call\n");
      int32_t addr;
      int sizep;	  
      addr = g_machine->ReadIntRegister(4);
      sizep = GetLengthParam(addr);
      ...
      int32_t sid = g_object_ids->AddObject(sem); 
    ...
    case SC_SEM_DESTROY:{
      DEBUG('e', (char*)"SEMAPHORE: SemDestroy call\n");
      int32_t sid = g_machine->ReadIntRegister(4);
      Semaphore * sem = (Semaphore *) g_object_ids->SearchObject(sid);
      if (sem && sem->typeId == SEMAPHORE_TYPE_ID)
	{
    ...
    case SC_LOCK_CREATE:{
      DEBUG('e', (char*)"LOCK: LockCreate call\n");
      int32_t addr;
      int sizep;	  
      addr = g_machine->ReadIntRegister(4);
      sizep = GetLengthParam(addr);
      ...
      int32_t sid = g_object_ids->AddObject(l); 
    ...
    case SC_LOCK_DESTROY:{
      DEBUG('e', (char*)"LOCK: LockDestroy call\n");
      int32_t sid = g_machine->ReadIntRegister(4);
      Lock * l = (Lock *) g_object_ids->SearchObject(sid);
      if (l && l->typeId == LOCK_TYPE_ID)
	{
	  g_object_ids->RemoveObject(sid);
	  delete (l);
	  g_syscall_error->SetMsg((char*)"",NoError);
	...
	{
	  g_syscall_error->SetMsg(msg, InvalidLockId);
	  g_machine->WriteIntRegister(2,-1);
	}
      break;
    }
    ...
    case SC_LOCK_ACQUIRE:{
      DEBUG('e', (char*)"LOCK: LockAcquire call\n");
      int32_t sid = g_machine->ReadIntRegister(4);
      Lock * l = (Lock *) g_object_ids->SearchObject(sid);
      if (l && l->typeId == LOCK_TYPE_ID)
	{
    ...
    case SC_LOCK_RELEASE:{
      DEBUG('e', (char*)"LOCK: LockRelease call\n");
      int32_t sid = g_machine->ReadIntRegister(4);
      Lock * l = (Lock *) g_object_ids->SearchObject(sid);
      if (l && l->typeId == LOCK_TYPE_ID)
	{
    ...
    case SC_COND_CREATE:{
      DEBUG('e', (char*)"COND: CondCreate call\n");
      int32_t addr;
      int sizep;	  
      addr = g_machine->ReadIntRegister(4);
      sizep = GetLengthParam(addr);
      ...
      int32_t sid = g_object_ids->AddObject(c); 
    ...
    case SC_COND_DESTROY:{
      DEBUG('e', (char*)"COND: CondDestroy call\n");
      int32_t sid = g_machine->ReadIntRegister(4);
