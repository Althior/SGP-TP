  ...
  for (i = 0 ; i < elfHdr.e_shnum ; i++)
	...
	continue;
      ...
      for (unsigned int pgdisk = 0,
	     pgmem = section_table[i].sh_addr / g_cfg->PageSize ;
	   ...
	   pgdisk++, pgmem ++)
	{
	  ...
	  else translationTable->clearBitWriteAllowed(pgmem);
	  ...
	  if (section_table[i].sh_type != SHT_NOBITS) {
	    ...
	    exec_file->ReadAt((char *)&(g_machine->mainMemory[translationTable->getPhysicalPage(pgmem)*g_cfg->PageSize]),
	    ...
	    // Fill it with zeroes
	    memset(&(g_machine->mainMemory[translationTable->getPhysicalPage(pgmem)*g_cfg->PageSize]),
		   0, g_cfg->PageSize);
	  }
	  ...
	  translationTable->clearBitSwap(pgmem);
	  translationTable->setBitReadAllowed(pgmem);
	  if (section_table[i].sh_flags & SHF_WRITE) translationTable->setBitWriteAllowed(pgmem);
	  else translationTable->clearBitWriteAllowed(pgmem);
	  translationTable->clearBitIo(pgmem);
...
#define STACK_BLANK_LEN 4 // in pages
  ...
  for (int i = stackBasePage ; i < (stackBasePage + numPages) ; i++) {
    ...
    translationTable->setPhysicalPage(i,g_physical_mem_manager->AddPhysicalToVirtualMapping(this,i));
    if (translationTable->getPhysicalPage(i) == -1) {
